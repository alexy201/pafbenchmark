...
def dynamically_modify_train_config(config: DictConfig):
    with open_dict(config):
        ...
        assert dataset_name in {'gen1', 'gen4', 'paf_event'}
        dataset_hw = get_dataloading_hw(dataset_config=dataset_cfg)

        model_cfg = config.encoder_model
        ...
        if mdl_name == 'rnndet':
            ...
            if backbone_name == 'MaxViTRNN':
                ...
                encoder_attention_cfg = config.encoder_model.encoder.attention
                ...
                encoder_attention_cfg.partition_size = partition_size
            else:
        ...
        partition_split_32 = encoder_cfg.partition_split_32
        assert partition_split_32 in (1, 2, 4)

        multiple_of = 32 * partition_split_32
        model_hw = _get_modified_hw_multiple_of(hw=dataset_hw, multiple_of=multiple_of)
        print(f'Set encoder (height, width) to {model_hw}')
        encoder_cfg.in_res_hw = model_hw
